...
class TestDCSpatialBaseline:
    ...
    def test_streets_parquet_exists_or_fetchable(self):
        """
        dc_streets.parquet is generated by src/fetch_dc_data.py but is not
        committed to the repo because it is not consumed by any current analysis
        script.  This test is skipped when the file is absent so that the full
        suite still passes in offline / CI environments; run
        ``python src/fetch_dc_data.py`` first to materialise the file.
        """
        streets_path = DATA_RAW / "dc_streets.parquet"
        if not streets_path.exists():
            pytest.skip(
                "dc_streets.parquet not present — run `python src/fetch_dc_data.py` "
                "to generate it (requires DC Open Data API access)"
            )
        gdf = gpd.read_parquet(streets_path)
        assert len(gdf) > 0
